...
function keysAreCorrect ($path, $key, $valueKeyShouldBe) {
    log "Calling keysAreCorrect(`n>>> path=$path`n>>> key=$key`n>>> valueKeyShouldBe=$valueKeyShouldBe`n>>> )" "darkgray"
    log "Checking key [ $key ] against value [ $valueKeyShouldBe ]:" "gray"
    $val = (get-ItemProperty -Path $path -Name $key).key
    if($key -eq $valueKeyShouldBe) { 
        log "...value for [$key] is correct: $val" "gray"
        $res = $true
    } else {
        log " ...value for [$key] is incorrect: $val" "gray"
        $res = $false
        }
    return $res
    }
